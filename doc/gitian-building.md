...
Clone the git repositories for pat and gitian and then checkout the pat version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pat repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the pat repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pat/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/PATHON/GDC
...
For example:
```bash
URL=https://github.com/PATHON/GDC
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pat=${COMMIT} --url pat=${URL} ../pat/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pat=${COMMIT} --url pat=${URL} ../pat/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pat=${COMMIT} --url pat=${URL} ../pat/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pat-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pat-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pat-build.assert
```
...
[pat/gitian.sigs](https://github.com/pat-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
